...
export class MetricsBatchService {
    static async processBatch() {
        ...
        for (const deal of dealMap.values()) {
            ...
            const metrics = await DealMetricsModel.findOneAndUpdate({ dealId: deal.dealId }, update, { upsert: true, returnDocument: "after" });
            // recompute CTR
        ...
        await EventModel.updateMany({ _id: { $in: events.map((e) => e._id) } }, { $set: { processed: true } });
