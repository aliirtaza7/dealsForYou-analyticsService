...
function toRecommendationAction(eventType) {
    switch (eventType) {
...
export class AnalyticsService {
    static getDealsServiceBaseUrl() {
        return process.env.DEALS_SERVICE_URL ?? "http://localhost:5002";
    }
    static async fetchDealsFromService(trendingMetrics) {
    ...
    static async updateDealMetrics(data) {
        ...
        const metrics = await DealMetricsModel.findOneAndUpdate({ dealId: data.dealId }, update, { upsert: true, returnDocument: 'after' });
        // recompute CTR
    ...
    static async getTrendingDeals() {
        ...
        const trendMap = new Map(trendingMetrics.map((item) => [String(item.dealId), item.currentTrendScore]));
...
AnalyticsService.TRENDING_LIMIT = 10;
